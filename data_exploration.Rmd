...
ggplot(state_freq, aes(state,freq, fill = state)) +
...
`account_length` has values ranging from -209 and 243. The variable `account_length` is not immediatley obvious what it represents but in the domain of this data set, `account_length` represents how long a customer has had an account in terms of days. With that being said `account_length` should not contain any negative values.
...
### 3.1 Turning Negatives into Positives
...
We have about `r sumNa$value` `NA` values in total in our dataset of which `account_length` owns 9.61% of it while `total_eve_minutes` and `total_intl_class` owns 3.22%.

<<<<<<< HEAD:data_discovery.Rmd
```{r}
...
=======
>>>>>>> 3a2837c007c28c064cda9a6cc0451f1a0fdfd1ec:data_exploration.Rmd
