...
For RandomForest, the proximity of the values is high within a range of 0 - 8 on the y-axis. As the value of y increases, the values are more widespread and dispersed.
...
For total_eve_minutes, as the value of the attribute (y) increases, the RandomForest values are distributed across a broad ranging between 100 & 1300 on the y-axis,
whereas for kNN, the values lie within a considerably narrow band of 100 - 350 on the y-axis. The increase in the value of y, has no effect of the values of this attribute.
...
Using the 'backwards' option, we proceed to identify the variables that have a significant impact in determining 'churn', using the kNN imputed values.
...
In other words, `international_plan` was the best predictor of churn followed by `number_customer_service_calls` etc. In backward, starting out with the full model, the single best predictor was `international_plan`.  IThis procedure was used to help in the creation of a best predicted model for churn as the dependent variable.
...
The table below shows the final models obtained from performing step wise regression in __backward__ direction. On the left we have he model using the Rainforest imputed missing values and on the right we have a model using the kNN imputed missing values.
...
The table below shows the final models obtained from performing step wise regression in __both__ direction. On the left we have he model using the Rainforest imputed missing values and on the right we have a model using the kNN imputed missing values.
...
The variables that are not common to all four models are:
