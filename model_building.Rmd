...
Loading libraries Magrittr which structuring sequences of data operations left-to-right,avoiding nested function calls and making it easy to add steps anywhere in the sequence of operations. The ROCR creates a visuialization of ROC graphs, sensitivity/specificity curves, lift charts,
and precision/recall plots are popular examples of trade-off
visualizations for specific pairs of performance measures. Finally pROC is described as a Tool for visualizing, smoothing and comparing receiver operating characteristic (ROC curves).
...
```
The Create_data_partition creates a function that splits the data into training 70% and having the reminder of 30%, this is then followed by dividing the data values into train and test. 
The "retutn" function will then creates a randomly formula to indentifies the rows equal to the sample size from all of the dataset and then stores these findings (row number) into "train_ind"
