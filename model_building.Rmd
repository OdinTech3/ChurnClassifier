...
```{r}
modelRF <- glm(churn~international_plan+voice_mail_plan+total_day_charge+total_eve_charge+total_intl_calls+number_customer_service_calls+total_day_minutes+total_eve_minutes+total_night_minutes+total_intl_charge+state,data=cdf_rf.imputed,family="binomial")
summary(modelRF)
```

```{r}
modelKNN <- glm(churn~international_plan+voice_mail_plan+total_day_charge+total_day_minutes+total_intl_calls+number_customer_service_calls+total_eve_minutes+total_eve_charge+total_night_minutes+total_intl_charge+state,data=cdf_knn.imputed,family="binomial")
summary(modelKNN)
```
...
#
#Prediction
pred1 = predict(mylogit_df1,val, type = "response")
finaldata = cbind(val, pred)
#
#Storing Model Performance Scores
pred_val <-prediction(pred ,finaldata$churn)
